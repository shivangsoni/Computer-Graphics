...
float ymax=0,ymin=1,xmax=0,xmin=1;
int c=0;
float zmin,zmax;
float x;
int cc=0;
...
void XZortho();
void YZortho();
float zpainter[10000][10000];
float ypainter[10000][10000];
float xpainter[10000][10000];

/****set in main()****/
//the number of pixels in the grid

float xvertex1[10000];
float yvertex1[10000];
float zvertex1[10000];


float xvertex2[10000];
float yvertex2[10000];
float zvertex2[10000];

float xvertex3[10000];
float yvertex3[10000];
float zvertex3[10000];
...
void draw_pix1(float x,float y,float z,float q);
...
for(int i=0;i<numpoly;i++)
  ...
  /*for (int i=0; i<numpoly; i++)
  {
    ...
    {
      for(int k=0;k<vertex[i]-j-1;k++)
      {  if (ypainter[i][k] < ypainter[i][k+1]) 
          {
           float swap       = ypainter[i][k];
           ypainter[i][k] = ypainter[i][k+1];
           ypainter[i][k+1] = swap;
...
void ints(int ras,float z)
{
  ...
  for(i=0;i<vertex[ras-1];i++)
  {
   ...
   if(vertexpoint1[ras-1][i]==3 && vertexpoint2[ras-1][i]==4)
   {
   //printf("\n%f     %f      %f        %f\n",x1,y1,x2,y2);
   }
   // printf("\n Vertex Point\n");
...
void bresenham(float x0, float y0, float x1, float y1) {
   ...
   /* float dx = x2 - x1, dy = y2 - y1;
    ...
    xIncr =   float(steps)/float(dx);
    yIncr =  float(steps)/float(dy);
    draw_pix(x, y);
    //printf("%f",steps);
    for (k = 0; k < steps; k=k+0.01) {
         x += xIncr; y += yIncr;
         printf("%f    ,     %f\n",x,y);
         draw_pix(x, y);
      //printf("Hi\n");
...
void HalfToning()
{
 for(int i=0;i<numpoly;i++)
 {
  for(int j=0;j<vertex[i];j++)
  {
    polyx[i][vertexpoint1[i][j]-1]=1.5*polyx[i][vertexpoint1[i][j]-1];
    polyy[i][vertexpoint1[i][j]-1]=1.5*polyy[i][vertexpoint1[i][j]-1];
    polyz[i][vertexpoint1[i][j]-1]=1.5*polyz[i][vertexpoint1[i][j]-1];
    
     polyx[i][vertexpoint2[i][j]-1]=1.5*polyx[i][vertexpoint2[i][j]-1];
    polyy[i][vertexpoint2[i][j]-1]=1.5*polyy[i][vertexpoint2[i][j]-1];
    polyz[i][vertexpoint2[i][j]-1]=1.5*polyz[i][vertexpoint2[i][j]-1];
    
        polyx[i][vertexpoint3[i][j]-1]=1.5*polyx[i][vertexpoint3[i][j]-1];
    polyy[i][vertexpoint3[i][j]-1]=1.5*polyy[i][vertexpoint3[i][j]-1];
    polyz[i][vertexpoint3[i][j]-1]=1.5*polyz[i][vertexpoint3[i][j]-1];   
  ...
  for(int i=0;i<numpoly;i++)
 {
  for(int j=0;j<vertex[i];j++)
  {
    polyx[i][vertexpoint1[i][j]-1]=polyx[i][vertexpoint1[i][j]-1]/1.5;
    polyy[i][vertexpoint1[i][j]-1]=polyy[i][vertexpoint1[i][j]-1]/1.5;
    polyz[i][vertexpoint1[i][j]-1]=polyz[i][vertexpoint1[i][j]-1]/1.5;
    
     polyx[i][vertexpoint2[i][j]-1]=polyx[i][vertexpoint2[i][j]-1]/1.5;
    polyy[i][vertexpoint2[i][j]-1]=polyy[i][vertexpoint2[i][j]-1]/1.5;
    polyz[i][vertexpoint2[i][j]-1]=polyz[i][vertexpoint2[i][j]-1]/1.5;
    
    polyx[i][vertexpoint3[i][j]-1]=polyx[i][vertexpoint3[i][j]-1]/1.5;
    polyy[i][vertexpoint3[i][j]-1]=polyy[i][vertexpoint3[i][j]-1]/1.5;
    polyz[i][vertexpoint3[i][j]-1]=polyz[i][vertexpoint3[i][j]-1]/1.5;   
...
void draw_pix1(float x1,float y1,float x2,float y2)
{
  glBegin(GL_POINTS);
  glColor3f(.5,.5,.5);
    float dx = x2 - x1, dy = y2 - y1;
    float steps = abs(dx) > abs(dy) ? abs(dx) : abs(dy), k;
    float xIncr, yIncr, x = x1, y = y1;
