   ...
   //    printf("point on the line %d,%d,%d,%d",polyx[i],polyy[i],polyx[i+1],polyy[i+1]);
    ...
    glutSwapBuffers();
     // do_cal(x4,y4,x1,y1);
      ...
      {
   //    printf("point on the line %d,%d,%d,%d",polyx[i],polyy[i],polyx[i+1],polyy[i+1]);
       do_cal(polyx[i][j],polyy[i][j],polyx[i][j+1],polyy[i][j+1]);
      }
     // printf("point on the line %d,%d,%d,%d",polyx[v-1],polyy[v-1],polyx[0],polyy[0]);
       do_cal(polyx[i][v[i]-1],polyy[i][v[i]-1],polyx[i][0],polyy[i][0]);
      }
       waitFor(5);      
       break;
       case 6:
	...
	scanf("%f",&ty);
     ...
     //draw_pix(polyx[0]+tx,polyy[0]+ty);
       for(int i=0;i<v[tid-1]-1;i++)
      {
       ...
       do_cal(polyx[tid-1][i]+tx,polyy[tid-1][i]+ty,polyx[tid-1][i+1]+tx,polyy[tid-1][i+1]+ty);
      }
      do_cal(polyx[tid-1][v[tid-1]-1]+tx,polyy[tid-1][v[tid-1]-1]+ty,polyx[tid-1][0]+tx,polyy[tid-1][0]+ty);
...
void do_cal(float x1,float y1,float x2,float y2)
{
    ...
    printf("Inside DDA line drawing algo code\n");
    dX = x1 - x2;
    dY = y1 - y2;
...
void sort(int ras,int z)
{
 int temp,i,j;
  ...
  {
   do_cal(polyx[ras-1][i],polyy[ras-1][i],polyx[ras-1][i+1],polyy[ras-1][i+1]);
  }
   do_cal(polyx[ras-1][v[ras-1]-1],polyy[ras-1][v[ras-1]-1],polyx[ras-1][0],polyy[ras-1][0]);
  for(i=0;i<c;i+=2)
  {
   do_cal(inter[ras-1][i],z,inter[ras-1][i+1],z);
  }
}
...
void rasterize(int ras)
{
  ...
  printf("Enter the polygon ID which you want to rasterize\n");
   int rastid;
   scanf("%d",&rastid);
  for(int i=0;i<v[rastid-1];i++)
  {
   if(xmin>polyx[rastid-1][i])
   {
    xmin=polyx[rastid-1][i];
   }
   if(xmax<polyx[rastid-1][i])
   {
    xmax=polyx[rastid-1][i];
   }
    if(ymin>polyy[rastid-1][i])
   {
    ymin=polyy[rastid-1][i];
   }
   if(ymax<polyy[rastid-1][i])
   {
    ymax=polyy[rastid-1][i];
   }  
  }
  
  for(int i=0;i<v[rastid-1];i++)
  {
   grid_matrix[(int)polyx[rastid-1][i]][(int)polyy[rastid-1][i]]=1;
  }
  for(int i=0;i<v[rastid-1]-1;i++)
  {
   do_cal2(polyx[rastid-1][i],polyy[rastid-1][i],polyx[rastid-1][i+1],polyy[rastid-1][i+1]);
  }
   do_cal2(polyx[rastid-1][v[rastid-1]-1],polyy[rastid-1][v[rastid-1]-1],polyx[rastid-1][0],polyy[rastid-1][0]);
...
void Scaling()
{
    ...
    ycentroid=ycentroid/float(v[scaleid-1]);
     //draw_pix(scalex*(polyx[scaleid-1][0]-xcentroid)+xcentroid,scaley*(polyy[scaleid-1][0]-ycentroid)+ycentroid);
      ...
      {
       do_cal(scalex*(polyx[scaleid-1][i]-xcentroid)+xcentroid,scaley*(polyy[scaleid-1][i]-ycentroid)+ycentroid,scalex*(polyx[scaleid-1][i+1]-xcentroid)+xcentroid,scaley*(polyy[scaleid-1][i+1]-ycentroid)+ycentroid);
      }
      do_cal(scalex*(polyx[scaleid-1][v[scaleid-1]-1]-xcentroid)+xcentroid,scaley*(polyy[scaleid-1][v[scaleid-1]-1]-ycentroid)+ycentroid,scalex*(polyx[scaleid-1][0]-xcentroid)+xcentroid,scaley*(polyy[scaleid-1][0]-ycentroid)+ycentroid);
