...
void display()
{
  ...
  /*  for (int j = 0; j < grid_height; j+=2){
   ...
   //    printf("point on the line %d,%d,%d,%d",polyx[i],polyy[i],polyx[i+1],polyy[i+1]);
     ...
     // do_cal(x4,y4,x1,y1);
      rasterize();
       printf("Do you want translation\n");
       scanf("%c",&cc);      
       if(cc == 'y')
       {
        Translation();
...
void Translation()
{
	...
	scanf("%f",&ty);
     draw_pix(polyx[0]+tx,polyy[0]+ty);
       for(int i=0;i<v-1;i++)
      {
       do_cal(polyx[i]+tx,polyy[i]+ty,polyx[i+1]+tx,polyy[i+1]+ty);
      }
      do_cal(polyx[v-1]+tx,polyy[v-1]+ty,polyx[0]+tx,polyy[0]+ty);
}
void Scaling()
{
     //clears the screen
    ...
    ycentroid=ycentroid/float(v);
     draw_pix(scalex*(polyx[0]-xcentroid)+xcentroid,scaley*(polyy[0]-ycentroid)+ycentroid);
       for(int i=0;i<v-1;i++)
      {
       do_cal(scalex*(polyx[i]-xcentroid)+xcentroid,scaley*(polyy[i]-ycentroid)+ycentroid,scalex*(polyx[i+1]-xcentroid)+xcentroid,scaley*(polyy[i+1]-ycentroid)+ycentroid);
      }
      do_cal(scalex*(polyx[v-1]-xcentroid)+xcentroid,scaley*(polyy[v-1]-ycentroid)+ycentroid,scalex*(polyx[0]-xcentroid)+xcentroid,scaley*(polyy[0]-ycentroid)+ycentroid);
}
